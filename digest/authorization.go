...
func (ah *authorization) refreshAuthorization(dr *DigestRequest) (*authorization, error) {
	...
	//fmt.Println("response ", ah.Response)
	return ah, nil
}
...
func (ah *authorization) computeResponse(dr *DigestRequest) (s string) {
	...
	ha1 = ah.computeA1(dr)
	ha2 = ah.computeA2(dr)
	kdSecret := ah.hash(ha1)
	//fmt.Println("HA1 ", ha1, " ", kdSecret)
	ksSecret := ah.hash(ha2)
	//fmt.Println("HA2 ", ha2, " ", ksSecret)

	kdData := fmt.Sprintf("%s:%08x:%s:%s:%s", ah.Nonce, ah.Nc, ah.Cnonce, ah.Qop, ksSecret)
	//fmt.Println("Data", kdData)
	return ah.hash(fmt.Sprintf("%s:%s", kdSecret, kdData))
}

func (ah *authorization) computeA1(dr *DigestRequest) string {

	if ah.Algorithm == "" || ah.Algorithm == "MD5" || ah.Algorithm == "SHA-256" || ah.Algorithm == "\"MD5\"" {
...
func (ah *authorization) hash(a string) (s string) {
	...
	var h hash.Hash
	//fmt.Println(a)
	if ah.Algorithm == "" || ah.Algorithm == "MD5" || ah.Algorithm == "MD5-sess" || ah.Algorithm == "\"MD5\"" {
		h = md5.New()
