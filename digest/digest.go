...
func (da *DigestAuth) CheckAuth(r *http.Request) (username string, authinfo *string) {
	...
	if da.Opaque != auth["opaque"] || auth["algorithm"] != "MD5" || auth["qop"] != "auth" {
		fmt.Println("Error 002", da.Opaque, auth["opaque"], auth["algorithm"], auth["qop"])
		return "", nil
	}
	//fmt.Println("002")
	...
	if r.RequestURI != auth["uri"] {
		...
		case !strings.HasPrefix(r.URL.Path, u.Path):
			fmt.Println("Error 006")
			return "", nil
		}
	}
	//fmt.Println("003", H("abc"))
	...
	//fmt.Println(strings.Join([]string{HA1, auth["nonce"], auth["nc"], auth["cnonce"], auth["qop"], HA2}, ":"))
